...
- 应用层：FastAPI 启动事件中执行数据库初始化；路由中通过依赖注入使用数据库会话
...
## 架构总览
下图展示从应用启动到数据库迁移、模型加载与请求处理的端到端流程：

```mermaid
sequenceDiagram
participant App as "应用启动"
participant Main as "main.on_startup"
...
### 数据库初始化与种子数据策略
- 初始化流程
  - 应用启动事件中调用迁移初始化，确保 schema 与版本一致
  - 通过 Alembic 升级到 head，避免直接使用 metadata.create_all
- 种子数据
...
## 结论
本项目采用标准化的 SQLAlchemy + Alembic 方案，实现了可演进的数据库架构：通过依赖注入管理连接、通过迁移工具控制版本、通过模型层表达表关系与约束。结合合理的索引策略与事务控制，可在保证一致性的同时兼顾性能。建议后续补充种子数据与监控告警机制，进一步完善生产级数据库管理能力。
