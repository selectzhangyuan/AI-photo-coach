...
- [apps/api/tests/test_auth_password_limits.py](file://apps/api/tests/test_auth_password_limits.py)
- [apps/api/tests/test_config.py](file://apps/api/tests/test_config.py)
...
## 更新摘要
**所做变更**
- 新增专门的密码安全测试文件测试策略章节，涵盖长密码、表情符号密码和最小长度限制测试
- 新增配置测试文件测试策略章节，涵盖数据库URL构建和密码字符处理
- 更新核心组件分析，增加密码安全处理机制说明
- 更新安全模块测试策略，强调SHA-256预哈希和bcrypt集成
...
## 项目结构
后端采用FastAPI应用，按功能模块组织，测试位于apps/api/tests目录下，分别覆盖认证、安全工具、配置管理和用户模块。前端位于apps/web，采用Vite+Vue+TypeScript；工作流服务由Celery任务队列支撑，分析流程由AI服务子模块协作完成。
...
T_pass_limits["tests/test_auth_password_limits.py"]
...
A_core_sec --> A_core_cfg
A_srv_ai --> A_srv_cv
A_srv_ai --> A_srv_llm
A_srv_ai --> A_srv_rules
A_srv_ai --> A_srv_comp
T_auth --> A_mod_auth
T_pass_limits --> A_core_sec
T_config --> A_core_cfg
T_users --> A_mod_users
T_sec --> A_core_sec
```
...
- [apps/api/app/core/security.py:1-92](file://apps/api/app/core/security.py#L1-L92)
- [apps/api/app/core/config.py:1-73](file://apps/api/app/core/config.py#L1-L73)
...
- [apps/api/tests/test_auth_password_limits.py:1-72](file://apps/api/tests/test_auth_password_limits.py#L1-L72)
- [apps/api/tests/test_config.py:1-22](file://apps/api/tests/test_config.py#L1-L22)
...
- 认证与安全
  ...
  - 密码安全处理：SHA-256预哈希确保bcrypt输入不超过72字节，支持表情符号和超长密码
- 用户模块
  - 用户资料查询、更新、改密等端点
  - 用户模型字段与约束
- 配置管理
  - 数据库URL构建：支持特殊字符密码的安全URL构建
  - 环境变量配置与验证
- AI分析流水线
  - 特征提取、LLM分析、规则编辑规划、结果合成
- 测试现状
  - 认证模块：注册/登录/刷新/登出/忘记密码等端点的集成测试
  - 密码安全测试：长密码、表情符号密码和最小长度限制的专门测试
  - 配置测试：数据库URL构建和密码字符处理的单元测试
...
### 密码安全测试策略（新增）
- 测试范围
  - 长密码支持：超过72字节的密码应被接受（通过SHA-256预哈希）
  - 表情符号密码：多字节表情符号密码应被接受
  - 最小长度限制：8字符最小长度仍需强制执行
- 密码安全机制
  - SHA-256预哈希：确保bcrypt输入不超过72字节
  - 兼容性：支持新格式（SHA-256预哈希）和旧格式（直接bcrypt）
  - 字符编码：UTF-8编码确保表情符号正确处理
- 测试策略
  - 使用TestClient进行端到端测试
  - 验证422状态码不包含"最多72"的错误消息
  - 确保最小长度验证仍然生效

```mermaid
flowchart TD
Input["输入密码"] --> PreHash["SHA-256预哈希"]
PreHash --> Bcrypt["bcrypt哈希"]
Bcrypt --> Store["存储哈希"]
Store --> Verify["验证密码"]
Verify --> NewFormat["新格式：SHA-256预哈希"]
Verify --> LegacyFormat["旧格式：直接bcrypt"]
NewFormat --> Success["验证成功"]
LegacyFormat --> Success
```
...
- 测试范围
  - 数据库URL构建：从组件构建完整URL
  - 特殊字符处理：密码中的特殊字符（如@@）正确保留
  - URL解析验证：用户名、密码、主机、数据库名称正确解析
- 配置管理机制
  - Settings类：集中管理所有配置项
  - URL构建：使用SQLAlchemy URL.create构建数据库连接URL
  - 环境变量支持：.env文件读取和UTF-8编码
- 测试策略
  - 使用pydantic的make_url解析构建的URL
  - 验证所有配置组件正确传递到最终URL
  - 确保特殊字符在URL中正确编码
...
Settings --> DatabaseURL : "构建"
```

**图表来源**
- [apps/api/app/core/config.py:8-60](file://apps/api/app/core/config.py#L8-L60)
...
- 设计原则
  ...
  - 密码安全测试：验证SHA-256预哈希和bcrypt集成的正确性
...
**图表来源**
- [apps/api/app/core/security.py:12-92](file://apps/api/app/core/security.py#L12-L92)
...
- 密码安全依赖：安全模块的SHA-256预哈希功能确保bcrypt输入长度限制，支持表情符号和超长密码。
- 配置依赖：配置模块负责构建数据库URL，支持特殊字符密码的正确处理。
- 测试对依赖的控制
  - 使用依赖覆盖将数据库会话替换为测试会话
...
- 安全工具异常
  ...
  - 检查密码哈希算法兼容性（新格式vs旧格式）
- 配置异常
  - 核对数据库URL构建是否正确；检查特殊字符密码的URL编码
  - 验证环境变量读取和UTF-8编码
- AI分析异常
...
- [apps/api/app/core/security.py:49-92](file://apps/api/app/core/security.py#L49-L92)
- [apps/api/tests/test_config.py:6-22](file://apps/api/tests/test_config.py#L6-L22)

## 结论
本项目已具备良好的测试基础：认证与用户模块的集成测试覆盖关键路径，安全工具的单元测试保障核心加密与令牌逻辑，AI分析服务的模块化便于独立测试。新增的密码安全测试和配置测试进一步完善了测试覆盖范围。建议在现有基础上补充：覆盖率目标、性能基准、安全扫描与端到端测试，完善CI流程与报告。
...
- 数据库
  ...
  - 密码安全测试使用SQLite内存库，支持表情符号和超长密码测试
  - 配置测试使用Settings类进行URL构建和解析验证

**章节来源**
- [apps/api/tests/test_auth.py:13-46](file://apps/api/tests/test_auth.py#L13-L46)
- [apps/api/tests/test_auth_password_limits.py:11-43](file://apps/api/tests/test_auth_password_limits.py#L11-L43)
- [apps/api/tests/test_config.py:1-22](file://apps/api/tests/test_config.py#L1-L22)
...
- 原则
  ...
  - 兼容性：测试新旧密码格式的兼容性
  - 特殊字符：测试URL构建中特殊字符的处理
- 覆盖率
  - 建议目标：函数级≥80%，分支级≥60%
  - 使用pytest-cov或coverage.py生成报告并接入CI
  - 密码安全测试覆盖率：确保SHA-256预哈希和bcrypt集成的完整覆盖
...
- 最佳实践
  ...
  - 密码测试：包含边界值（72字节、73字节、表情符号数量）
  - 配置测试：验证特殊字符在URL中的正确处理
- 调试技巧
  - 在测试中打印关键中间结果（如令牌、特征字典）
  - 使用pytest --pdb快速定位异常
  - 逐步缩小问题范围：先单元后集成，再端到端
  - 密码安全问题：检查SHA-256预哈希和bcrypt的组合流程
  - 配置问题：验证URL构建和解析的每一步