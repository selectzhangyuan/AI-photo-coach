...
## 更新摘要
**所做更改**
- 新增认证界面完整文档，包括登录、注册、个人资料管理页面
- 更新状态管理章节，详细说明Pinia认证状态管理集成
- 新增API层认证接口文档
- 更新架构图表，反映认证界面与状态管理集成
- 完善导航守卫与路由权限控制说明
...
**更新** 新增完整的认证界面，包括登录、注册、个人资料管理页面，以及与Pinia状态管理的深度集成。
...
**更新** 新增认证界面组件与状态管理集成，完善导航守卫与权限控制。
...
- Token过期：HTTP拦截器会自动刷新Token，若失败则需重新登录
...
## 结论
本应用以清晰的分层架构实现了照片上传与 AI 分析、用户认证与个人资料管理等核心功能。通过 Pinia 管理认证与分析状态，Vue Router 控制页面流转，全局样式系统保证视觉一致性。新增的认证界面提供了完整的用户身份验证流程，包括登录、注册、个人资料管理等功能。开发与构建流程简洁高效，具备良好的扩展性与维护性。

## 附录
- 开发命令：dev、build、preview
- 开发服务器：host=0.0.0.0，port=5151，代理 /api 到后端
- 主题变量：可在全局 CSS 中调整颜色与尺寸变量以定制主题
- 认证流程：本地存储Token，自动注入Authorization头，401时自动刷新
